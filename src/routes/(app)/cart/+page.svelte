<script lang="ts">
	...
	const embType = [
		'flat', 'cap', 'applique', 'nameTag'
	]

	let activitiesTabs = [
		{ id: uuidv4(), name: 'Products', selected: true },
		{ id: uuidv4(), name: 'Customer Details', selected: false }
	];
...
<div class="flex-grow flex overflow-x-hidden">
	...
	<div class="flex-grow bg-white dark:bg-gray-900 overflow-y-auto">
		...
		{#if selectedActivitiesTab.name === 'Products'}
										...
										>
											{#each embType as type (type)}
												{#if !( type === product.embroidery_type)}
													<li>
														<button on:click={() => cartStore.changeEmbType({id: key, type})}  class="rounded-sm">
															{type}
														</button>
													</li>
												{/if}
								...
								<td class="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800">
									<div class="flex items-center">
										<button
											...
											class="bg-slate-600 px-2 hover:bg-blue-500"
										>
											<span>-</span>
										</button>
										<div class="px-3">
											<span
												>{$cartStore.has(product.id)
													? $cartStore.get(product.id).quantity
													: 0}</span
											>
										</div>
										<button
		...
		{/if}
		{#if selectedActivitiesTab.name === 'Customer Details'}
